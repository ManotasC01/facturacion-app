...
function VencimientoChip({ label, fecha }) {
  if (!fecha) return null;
  const dias = diasRestantes(fecha);
  const vencido  = dias < 0;
  const urgente  = dias >= 0 && dias <= 30;
  const proximo  = dias > 30 && dias <= 90;
  const bg    = vencido ? "#fce4ec" : urgente ? "#fff3e0" : proximo ? "#fff8e1" : "#e8f5e9";
  const color = vencido ? "#c62828" : urgente ? "#e65100" : proximo ? "#f57f17" : "#2e7d32";
  const icono = vencido ? "⛔" : urgente ? "⚠️" : proximo ? "🕐" : "✅";
  const texto = vencido
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5, background:bg,
      borderRadius:8, padding:"5px 9px", marginTop:5 }}>
      <span style={{ fontSize:12 }}>{icono}</span>
      <div>
        <div style={{ fontSize:10, fontWeight:700, color, letterSpacing:0.3 }}>{label.toUpperCase()}</div>
...
export default function App() {
  ...
  const [formPaquete, setFormPaquete] = useState(FORM_PAQUETE_INIT);
  ...
  useEffect(() => {
    ...
    const unsub = escucharClientes((data) => {
      setClientes(data);
      setSyncStatus("ok");
      setClienteSel(sel => sel ? (data.find(c => c.id === sel.id) || sel) : sel);
    });
    return () => unsub();
  }, []);
  ...
  const clientesFiltrados = useMemo(() => {
    ...
    return clientes.filter(c =>
      c.nit.toLowerCase().includes(q) || c.nombre.toLowerCase().includes(q)
    );
  }, [clientes, busqueda]);
  ...
  async function guardarClienteHandler() {
    ...
    try {
      if (clienteSel && vista==="detalle") {
        const u = { ...clienteSel, ...formCliente };
        await guardarCliente(u); setClienteSel(u);
        showToast("Cliente actualizado ✓");
      } else {
        const n = { id:uid(), ...formCliente, paquetes:[], creadoEn:new Date().toISOString() };
        await guardarCliente(n);
  ...
  async function eliminarClienteHandler(id) {
    setSyncStatus("saving");
    try { await eliminarCliente(id); showToast("Cliente eliminado"); }
  ...
  function abrirModalPaquete(p=null) {
    ...
    setFormPaquete(p ? { ...FORM_PAQUETE_INIT, ...p } : { ...FORM_PAQUETE_INIT });
  ...
  // Alertas de vencimiento próximo para badge en lista
  function alertasPaquetes(paquetes) {
    return paquetes.filter(p => {
      const dc = p.vencCertificado   ? diasRestantes(p.vencCertificado)   : null;
      const dt = p.vencTransacciones ? diasRestantes(p.vencTransacciones) : null;
      return (dc !== null && dc <= 30) || (dt !== null && dt <= 30);
    }).length;
  }
  ...
  const fp = formPaquete;
  const setFP = (k,v) => setFormPaquete(prev => ({ ...prev, [k]:v }));
    ...
    <div style={s.root}>
      ...
      <div style={s.statusBar}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5 }}>
          <div style={{ width:7, height:7, borderRadius:"50%", background:syncInfo.color,
            animation: syncInfo.pulse ? "pulse 1s infinite" : "none" }} />
          <span style={{ fontSize:10, color:"rgba(255,255,255,0.9)", fontWeight:600 }}>{syncInfo.label}</span>
        </div>
      </div>
              ...
              <>
                ...
                <div style={s.list}>
                  {clientesFiltrados.map(c => {
                    ...
                    const alertas  = alertasPaquetes(c.paquetes);
                    ...
                    return (
                      <div key={c.id} style={s.card} onClick={() => abrirDetalle(c)}>
                        ...
                        <div style={s.cRight}>
                          ...
                          {alertas>0  && <div style={s.alertaBadge}>⚠️ {alertas}</div>}
                    ...
                    return (
                      <div key={pref} style={s.prefijoCard}>
                        <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                          ...
                          {alertas>0 && <span style={{ fontSize:13 }}>⚠️</span>}
                        </div>
                ...
                {clienteSel.paquetes.map(p => {
                  ...
                  return (
                    <div key={p.id} style={s.paqCard}>
                      ...
                      {/* Vencimientos */}
                      <VencimientoChip label="Venc. Certificado"    fecha={p.vencCertificado} />
                      <VencimientoChip label="Venc. Transacciones"  fecha={p.vencTransacciones} />
          ...
          <div style={{ ...s.sheet, maxHeight:"92vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
            ...
            <Fld label="Prefijo">
              <input style={s.inp} placeholder="Ej: FE, NC, DIST..."
                value={fp.prefijo} onChange={e => setFP("prefijo", e.target.value.toUpperCase())}/>
            </Fld>

            <div style={{ display:"flex", gap:10 }}>
              <Fld label="Factura inicio" style={{ flex:1 }}>
                <input style={s.inp} type="number" placeholder="1001"
                  value={fp.facturaInicio} onChange={e => setFP("facturaInicio", e.target.value)}/>
              </Fld>
              <Fld label="Factura fin" style={{ flex:1 }}>
                <input style={s.inp} type="number" placeholder="1100"
                  value={fp.facturaFin} onChange={e => setFP("facturaFin", e.target.value)}/>
              </Fld>
            </div>

            {fp.prefijo && fp.facturaInicio && fp.facturaFin && (
              <div style={s.preview}>
                <span style={{ color:"#1565c0", fontWeight:700, fontFamily:"monospace", fontSize:13 }}>
                  {fp.prefijo}-{String(fp.facturaInicio).padStart(8,"0")}
                </span>
                <span style={{ color:"#777", margin:"0 6px" }}>→</span>
                <span style={{ color:"#1565c0", fontWeight:700, fontFamily:"monospace", fontSize:13 }}>
                  {fp.prefijo}-{String(fp.facturaFin).padStart(8,"0")}
                </span>
                <div style={{ fontSize:11, color:"#777", marginTop:4 }}>
                  {Math.max(0, parseInt(fp.facturaFin)-parseInt(fp.facturaInicio)+1)} facturas
                </div>
              </div>
            )}

            {/* Separador vencimientos */}
            <div style={s.dividerLabel}>📅 Fechas de vencimiento</div>

            <div style={{ display:"flex", gap:10 }}>
              <Fld label="Venc. Certificado" style={{ flex:1 }}>
                <input style={s.inp} type="date"
                  value={fp.vencCertificado} onChange={e => setFP("vencCertificado", e.target.value)}/>
                {fp.vencCertificado && (
                  <div style={s.fechaHint}>
                    {(() => {
                      const d = diasRestantes(fp.vencCertificado);
                      return d < 0 ? `⛔ Vencido hace ${Math.abs(d)}d`
                           : d === 0 ? "⚠️ Vence hoy"
                           : d <= 30  ? `⚠️ ${d}d restantes`
                           : `✅ ${d}d restantes`;
                    })()}
                  </div>
                )}
              </Fld>
              <Fld label="Venc. Transacciones" style={{ flex:1 }}>
                <input style={s.inp} type="date"
                  value={fp.vencTransacciones} onChange={e => setFP("vencTransacciones", e.target.value)}/>
                {fp.vencTransacciones && (
                  <div style={s.fechaHint}>
                    {(() => {
                      const d = diasRestantes(fp.vencTransacciones);
                      return d < 0 ? `⛔ Vencido hace ${Math.abs(d)}d`
                           : d === 0 ? "⚠️ Vence hoy"
                           : d <= 30  ? `⚠️ ${d}d restantes`
                           : `✅ ${d}d restantes`;
            ...
            <Fld label="Fecha de asignación">
              <input style={s.inp} type="date"
                value={fp.fecha} onChange={e => setFP("fecha", e.target.value)}/>
            </Fld>
                ...
                {ESTADOS.map(est => {
                  ...
                  return (
                    <button key={est} style={{
                      ...
                      background: fp.estado===est ? ec.bg : "#f5f5f5",
                      border: `2px solid ${fp.estado===est ? ec.dot : "transparent"}`,
                      color: fp.estado===est ? ec.text : "#888",
                    }} onClick={() => setFP("estado", est)}>
                      {est}
                    </button>
                  );
                })}
            ...
            <Fld label="Descripción (opcional)">
              <input style={s.inp} placeholder="Ej: Paquete enero 2026"
                value={fp.descripcion} onChange={e => setFP("descripcion", e.target.value)}/>
            </Fld>
...
const s = {
  ...
  alertaBadge:{ background:"#fff3e0", color:"#e65100", fontSize:10, fontWeight:600, padding:"2px 6px", borderRadius:6, marginTop:2 },
  ...
  rangoRow:   { display:"flex", alignItems:"center", gap:6, background:"#f8f9ff", borderRadius:10, padding:"10px 12px", marginBottom:6 },
  ...
  dividerLabel:{ fontSize:12, fontWeight:700, color:"#1565c0", background:"#e3f2fd", borderRadius:8, padding:"6px 12px", marginBottom:13, textAlign:"center" },
  fechaHint:  { fontSize:11, color:"#888", marginTop:4, fontWeight:500 },
